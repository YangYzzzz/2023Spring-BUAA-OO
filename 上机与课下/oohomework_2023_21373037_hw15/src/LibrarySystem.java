...
public class LibrarySystem { //归还书籍未完成，整体流程启动未完成，继续优化！
        ...
        for (MyPair pair : PREPARE_LIST) { //对每一条指令几种可能 1 这个人不可以借这本书 pass; 2 可以从外校借书，从外校借；3 本校预定
            ...
            if (person.canBorrowBook(pair.getCategory(), pair.getName(),
                ...
                if (canGetBookFromOutside(person, library, category, name) == 2) { //能借的话已经在之中完成了借
                    System.out.println("(Sequence) " + getTime() +
                            " LibrarySystem sends a message to OrderingLibrarian");
                    library.getOrderingLibrarian().orderNewBook(person, category, name);
                }
            }
        }
        PREPARE_LIST.clear();
    ...
    public static void arrangeAllBook() {
        ...
        System.out.println("(Sequence) " + getTime() +
                " LibrarySystem sends a message to ArrangingLibrarian");
        for (Library library : LIBRARIES) {
    ...
    public static void purchaseAllBook() {
        System.out.println("(Sequence) " + getTime() +
                " LibrarySystem sends a message to PurchasingDepartment");
        for (Library library : LIBRARIES) {
    ...
    public static void addAllBookTime() {
        for (Person person : PEOPLE) {
            person.addBookTime();
        }
    }
